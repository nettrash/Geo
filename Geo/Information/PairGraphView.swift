...
struct PairGraphView: View {
    
    @State var Caption: String
    @Binding var Data: [PairDataItem]
    ...
    var body: some View {
        ZStack {
            ...
            DataSetShape(height: 230, shiftX: 30, shiftY: 30, data: Data.map({ pair in
                DataItem(Value: pair.Value0, Legend: pair.Legend)
            }), min: min, max: max, markVertexes: true, vertexRadius: 1.5)
                .strokeBorder(getColor(0), lineWidth: 1, antialiased: true)

            DataSetShape(height: 230, shiftX: 30, shiftY: 30, data: Data.map({ pair in
                DataItem(Value: pair.Value1, Legend: pair.Legend)
            }), min: min, max: max, markVertexes: true, vertexRadius: 1.5)
                .strokeBorder(getColor(1), lineWidth: 1, antialiased: true)
        }
    }
...
#Preview {
    PairGraphView(Caption: "TEST PAIR PREVIEW", Data: .constant([]), Lines: [GraphLine(value: 762, text: "normal", color: Color.green), GraphLine(value: 100, text: "yellow", color: Color.yellow), GraphLine(value: 400, text: "red", color: Color.red)], min: .constant(CGFloat.zero), max: .constant(CGFloat(1000.0)), measurement: "kPa")
}
